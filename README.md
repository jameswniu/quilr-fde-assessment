...
- These are the tests I built the code against, so they check it against itself. The SDK client and MCP Inspector never read them, and both got the same schema and the same refusal.
- I picked the seven bench openings after finding where the cost lived, so 583 ms is the worst of my own set rather than a sample of traffic. What travels between machines is the 53 chunks held.
