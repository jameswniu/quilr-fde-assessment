...
One card per number a reviewer sees, written from the code and from live runs rather than from the README. Audited on `master` at `0308589`, working tree clean, no remote. Two exceptions are the time to first token card and the held text by content shape card below, both of which describe a fix to `scripts/bench_stream.py` landing after `0308589` rather than the script as it stood there, and are audited against that fixed worktree instead. `make bench` at `0308589` alone still prints the old total time line and has no content shape table. The time to first token card has since been audited a second time, against the multi shape first token bench landing in this change, because through `0592102` that bench measured only the safe prose shape and reported its near zero result as if it were the whole story. That same change is what finally builds an input that nearly reaches the 640 character ceiling, so the hard ceiling card's Sibling and Bound lines are audited here too. The landing page figures, `tools/draw_figures.py`, `tools/check_claims.py` and the two reports they read all land after `17d269d`, so every card that mentions them is audited against this working tree rather than against an earlier commit.

The reader-facing surface is `README.md` with the three figures it carries, `docs/TASKS.md`, the thirteen Makefile help lines, the module docstrings, and what `make test`, `make lint`, `make bench` and `make run-task4` print. No figure carries a number that was typed by hand. `tools/draw_figures.py` reads every value it draws either from `reports/bench_report.json`, from `reports/test_report.json`, or from the constant in `src/` that defines it, which is where the system map gets the ports, the error codes, the 50,000 token budget and the 3000 ms timeout. `make figures-check` redraws and compares against what is committed, so a figure cannot drift from its generator.

The badges and the prose are the parts of the page still typed by hand, so `make claims` pins them. It collects the suite for the case and function counts, then runs it for the outcome counts, rereads `MAX_BUFFERED_CHARS` and `.python-version`, and fails when the three badges, the hero band or the hero alt text stops matching. It runs the suite rather than only collecting it because collection cannot see a skipped test and the hero band claims a skip count, and `make check` puts it before `figures-check` so the figures are compared against a report from this run rather than the last one. What it does not pin is the body prose, which repeats several of the same numbers, so a stale figure there is caught by reading rather than by a command. Every measurement a reviewer sees is still printed by a command they run themselves.
...
```
Claim     50,000 tokens a minute per tenant API key (docs/TASKS.md, Task 4, and the system map's limiter card, which reads rate_limiter.py line 31 DEFAULT_LIMIT_TOKENS at draw time)
...
```
Claim     3000 ms (README, What each task owns, docs/TASKS.md, Task 4, and the system map's router card, which reads router.py line 38 DEFAULT_TIMEOUT_MS at draw time)
...
```
Claim     "199 passed" from make test. On the page it is the tests badge, the hero band's first tile, the hero alt text, the system map's stat box and the "199 green cases" line under What this does not prove
...
Match     a case counts when pytest reports it green under pyproject.toml, testpaths tests, asyncio_mode auto. Skips, xfails and xpasses are counted separately by make claims, which fails when the number the hero band shows stops matching the run, so "0 skips" is a measured outcome rather than an assertion
...
Command   make test, which is uv run pytest. make claims collects and then runs it, writes both counts to reports/test_report.json, and fails when the badge, the hero band or the hero alt text disagrees with that run
...
Knob      one extra tuple in an existing parametrize decorator moves 199 without testing a single new behaviour, which is why the function count sits beside it. A skip marker moves it the other way, which is what the skip count beside it is for
```
...
Sibling   the highest value observed here is 636, four short of the ceiling, held while the first token bench streams its last shape, a 320 character address followed by "_" and 320 more token characters in 12 character chunks. make bench now prints that 636 on the line under the ceiling and writes it to reports/bench_report.json, so the hero band quotes it beside the ceiling and make claims fails if the alt text stops matching it. The other figure the bench prints is 320, from the 100,000 character unbroken run in the content shape table
...
Command   make bench, scripts/bench_stream.py for the instrument and the table, then make figures to redraw assets/first-token.svg and the hero band from reports/bench_report.json
...
Bound     one chunk size and one upstream cadence on one loaded laptop. The figure and the hero tile show whatever the last make bench measured, so the milliseconds there move by a few between runs while the waits column does not, and make figures-check reports that move as drift rather than hiding it. The wait is whole chunk delays, so a provider sending larger chunks clears the same window in fewer of them and a slower provider pays more, and no real provider is measured here. The last shape is the worst this design allows rather than a shape drawn from real traffic, since a match can only pull the cut back to its own start while fewer than 640 characters have arrived, and nothing here says how often any of the seven shapes occurs
...
- No exact witness for the 640 character ceiling. The first token bench's last shape does construct the straddling case and holds 636 of the 640 at 12 character chunks, so what is unproven is the last four characters rather than the whole second half.
