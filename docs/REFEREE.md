...
| Chosen before or after the result | Before. It is the brief's own example, sitting in `rate_limiter.py:31` as a default any caller can override, and there was never a result to fit it to. |
...
| Chosen before or after the result | Before. Also the brief's number, in `router.py:38`, and no run of anything picked it. |
...
| Chosen before or after the result | After, and this is the honest one. These are the tests the code was built against, so they cannot be independent of it. The two witnesses that are independent are the official SDK client in `tests/test_task1_sdk_client.py` and MCP Inspector, neither of which had read an assertion of mine. |
...
| Chosen before or after the result | Before. Nothing was sampled here. Every path either opens a socket or it does not, and the table below lists them one by one. |
...
| Chosen before or after the result | After. I ran it about twenty times and kept five, the ones whose load average matched the earlier reading, and I was looking at the timings while I chose. The load average 120 row below is what the discarded ones looked like. No page quotes this number. |
...
| Chosen before or after the result | Before for the four length rows, whose repeated prose and " ada@example.com" tail were fixed in the script's first cut. The three shape rows came after, once the flat result made it clear the tail was setting the number and not the length. |
...
| Chosen before or after the result | Before. It falls out of the caps in `patterns.py`, so it was computable before a bench existed. The 636 beside it came after, from an input built to press the bound. |
...
| Chosen before or after the result | After. I explored first, found the cost lived in what the response opens with rather than how long it is, then picked seven openings to bracket that. So 583 ms is the worst of a set I chose, not a survey of traffic. Earlier cuts of `scripts/bench_stream.py` timed safe prose alone, and one held text row turned out to be measuring its own fixture. |
...
| Chosen before or after the result | Before, and nothing rests on it either way. The fixture and the four sizes were fixed in the script's first cut, and 2.0 is simply what the committed run printed, inside the 2.0 to 6.6 KiB spread below. |
...
- `make lint` says 46 files formatted and 43 typed, and both are right. Ruff 0.16 formats Markdown too, so it counts `README.md`, `docs/REFEREE.md` and `docs/TASKS.md` on top of the 43 Python files mypy counts.
