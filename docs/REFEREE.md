...
| Chosen before or after the result | Before. The brief's own example, in `rate_limiter.py:31`, with no result to fit it to. |
...
| Chosen before or after the result | Before. The brief's number too, in `router.py:38`. No run picked it. |
...
| Chosen before or after the result | After. These are the tests the code was built against. The independent witnesses are the SDK client in `tests/test_task1_sdk_client.py` and MCP Inspector. |
...
| Chosen before or after the result | Before. Nothing was sampled. Every path either opens a socket or does not, and the table below lists them. |
...
| Chosen before or after the result | After. I ran it about twenty times and kept five, watching the timings while I chose. No page quotes it. |
...
| Chosen before or after the result | Before for the four length rows. The three shape rows came after, once the flat result showed the tail set the number. |
...
| Chosen before or after the result | Before, derived from the caps in `patterns.py`. The 636 came after, from an input built to press the bound. |
...
| Chosen before or after the result | After. I found the cost lived in the opening, then picked seven to bracket it, so 583 ms is the worst of my own set. |
...
| Chosen before or after the result | Before, and nothing rests on it. Fixture and sizes fixed in the first cut, 2.0 is what the committed run printed. |
